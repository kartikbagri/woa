...
class SelfAdaptiveWhaleOptimization:
    """class implements the whale optimization algorithm as found at
    https://link.springer.com/article/10.1007/s10586-017-1628-3#Sec8
    Implementing self adaptiveness in whale optimisation
    Modifications - change random probability p to k which is dependent on prev and curr fitness
    """
    ...
    def optimize(self, t, ngens, max_fe):
        ...
        for i in range(1, len(self._sols)):
            if self._fe >= max_fe:
                return

            s = self._sols[i]
            ft_1 = self._prev_fit[i]
            self._fe += 1
            ft = self._opt_func(s)
            t = (ft_1 - ft) / ft_1
            k = np.random.uniform(0.0, 1.0) * t
            self._prev_fit[i] = ft
            if k > 0.5:
                A = self._compute_A()
    ...
    def _init_solutions(self, nsols):
        ...
        for i in range(nsols + 1):
            self._prev_fit.append(np.random.uniform(0.0, 1.0))
        return sols
...
class SAWOA:
    ...
    def solve(self, problem_dict, termination):
        ...
        opt_alg = SelfAdaptiveWhaleOptimization(
            opt_func,
            constraints,
            self.pop_size,
            b,
